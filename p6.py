...
# ---------------------------
df = pd.read_csv(r'C:/Users/Archana/OneDrive/Attachments/Desktop/MDTM46B/Project 6 - Hyperlocal News Anamoly Detection/Cleaned_Data.xls', encoding='latin1')

@st.cache_resource
def load_models():
    scaler = joblib.load(r"C:/Users/Archana/OneDrive/Attachments/Desktop/MDTM46B/Project 6 - Hyperlocal News Anamoly Detection/scaler1.pkl")
    clf = joblib.load(r"C:/Users/Archana/OneDrive/Attachments/Desktop/MDTM46B/Project 6 - Hyperlocal News Anamoly Detection/XGBClassifier1.pkl")              # anomaly model
    loc_encoder = joblib.load(r"C:/Users/Archana/OneDrive/Attachments/Desktop/MDTM46B/Project 6 - Hyperlocal News Anamoly Detection/LabelEncoder1.pkl")
    loc_model = joblib.load(r"C:/Users/Archana/OneDrive/Attachments/Desktop/MDTM46B/Project 6 - Hyperlocal News Anamoly Detection/XGBClassifier.pkl")    # <— new model for location prediction
...
if selected == 'New':
    ...
    if st.button("🔍 Predict"):
        if article.strip() == "":
            ...
            loc_pred = loc_model.predict(embedding_np)
            location_name = loc_encoder.inverse_transform([loc_pred])[0]
            # 🔹 Step 2 — Combine embedding + location for anomaly prediction
            embedding_2d = embedding_np[0].reshape(1, -1)
            loc_2d = np.array([loc_pred]).reshape(1, -1)
            combined_features = np.concatenate([embedding_2d, loc_2d], axis=1)

            st.write("combined feature shape",combined_features.shape)
            # Step 3: Scale and predict anomaly
  
            if combined_features.shape[1] != scaler.n_features_in_:
                st.error(f"Feature mismatch! Scaler expects {scaler.n_features_in_} features, but got {combined_features.shape[1]}.")
            else:
                ...
                if prediction == 1:
                    st.error(f"⚠️ The article from *{location_name}* is *Anomalous* (prob={proba:.2f}).")
                else:
                    st.success(f"✅ The article from *{location_name}* is *Normal News* (prob={proba:.2f}).")